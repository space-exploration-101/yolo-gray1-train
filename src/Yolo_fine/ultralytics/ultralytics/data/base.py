...
class BaseDataset(Dataset):
    ...
    def _read_image(self, path):
        """Read an image according to the dataset input contract."""
        data = getattr(self, "data", {}) or {}
        channels = data.get("channels")
        semantics = data.get("input_semantics")
        gray1_requested = channels == 1 or semantics == "gray1"
        if gray1_requested and (channels != 1 or semantics != "gray1"):
            raise ValueError(
                "Single-channel datasets must declare both channels: 1 and input_semantics: gray1."
            )
        flags = cv2.IMREAD_GRAYSCALE if gray1_requested else cv2.IMREAD_COLOR
        im = cv2.imread(str(path), flags)
        if im is not None and gray1_requested and im.ndim != 2:
            raise ValueError(f"Expected a 2D grayscale image from {path}, got shape {im.shape}.")
        return im
    ...
    def cache_images_to_disk(self, i):
        ...
        f = self.npy_files[i]
        if not f.exists():
            np.save(f.as_posix(), self._read_image(self.im_files[i]), allow_pickle=False)
    ...
    def check_cache_ram(self, safety_margin=0.5):
        ...
        n = min(self.ni, 30)  # extrapolate from 30 random images
        for _ in range(n):
            im = self._read_image(random.choice(self.im_files))  # sample image
            if im is None:
                continue
