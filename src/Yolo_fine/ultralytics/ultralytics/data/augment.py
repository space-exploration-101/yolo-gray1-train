...
class RChannelOnly:
    """Keep OpenCV BGR images in the dataset's R-only grayscale representation."""

    def __call__(self, labels):
        img = labels["img"]
        if img.ndim != 3 or img.shape[2] != 3:
            raise ValueError(f"Expected a three-channel BGR image, got shape {img.shape}.")
        img[..., :2] = 0  # B=0, G=0; the existing R channel is the grayscale intensity.
        return labels
...
class Format:
    ...
    def _format_img(self, img):
        ...
        # RGB/BGR reversal is meaningful only for three-channel input. Reversing
        # a singleton channel axis creates a negative stride that PyTorch rejects.
