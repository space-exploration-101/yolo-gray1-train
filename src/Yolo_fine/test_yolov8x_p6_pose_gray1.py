"""Evaluate a grayscale-native YOLOv8 pose model on the held-out test split.
...
DEFAULT_OUTPUT = Path("/experiments/eval-gray1")
...
def predict_gray1_batches(
    model: YOLO,
    image_paths: list[Path],
    batch_size: int,
    predict_kwargs: dict[str, object],
):
    """Yield predictions from explicit HxWx1 grayscale arrays without a color reload."""
    for start in range(0, len(image_paths), batch_size):
        ...
        for path in paths:
            ...
            if gray is None:
                raise FileNotFoundError(f"Unable to read grayscale image: {path}")
            images.append(gray[..., None])
        results = model.predict(source=images, stream=False, **predict_kwargs)
        if len(results) != len(paths):
            raise RuntimeError(f"Predictor returned {len(results)} results for {len(paths)} gray1 inputs")
        yield from zip((path.resolve() for path in paths), results)
...
def run_custom_metrics(
    ...
    for image_path, result in predict_gray1_batches(model, image_paths, args.batch, predict_kwargs):
